...
def main():
    ...
    #serie html source code

    url = str(input(f'Url de la série ? - '))
    response = requests.get(url)
    html = response.text
    soup3 = BeautifulSoup(html, "html.parser")

    #liste pour stock les liens de chaque saison
    saison_links = []

    for tag in soup3.find_all(lambda tag: tag.name == "a" and not tag.find_parent("th") and tag.text.strip() != "Parent Directory"):
        saison_url = tag.get("href") #là il récup la fin du lien
        saison_urls = f'{url}{saison_url}' #là j'ajoute le http://films.lizziewizzie.site/"dossier-série"
        saison_links.append(saison_urls)

    nb_saisons = len(saison_links)
    
    x = 0
    
    for saison in range(nb_saisons):
        x += 1
        url = saison_links[saison]
        response = requests.get(url)
        html = response.text
