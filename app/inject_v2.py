...
def main():
    ...
    nom_film = str(input('Nom de la série ? - '))
    ...
    for saison in range(nb_saisons):
        ...
        nb_episodes = int(input(f"Combien d'épisodes Saison {x} ? - "))
        url_saison = str(input(f"Début de l'url des épisodes de la saison {x} ? (sans le champ répétitif) - "))
        url_extention = str(input("Fin de l'url et extension ? - "))

        y = 0

        for saisonbtn in range(nb_episodes):
            y += 1

            if y < 10:
                a_saison = soup.new_tag('a', href=f'{url_saison}0{y}{url_extention}')
            
            else:
                a_saison = soup.new_tag('a', href=f'{url_saison}{y}{url_extention}')
...
def ajout():
    ...
    nom_film = str(input("Nom du film déjà ? - "))
    with open('/home/frigiel/Documents/VSCODE/Django/Films/propterre/templates/propterre/series.html', 'r') as fichier:
        contenu = fichier.read()
